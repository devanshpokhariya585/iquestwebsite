...
// Empty by default → requests go to the same origin (Vite proxies /api to the
// server in dev; the Express server serves both in production). Only set
// VITE_API_URL if you host the API on a different origin.
      ...
      >
        ...
        >
          ...
          <input
            ...
            className="w-full bg-transparent py-2 font-mono text-sm text-ink placeholder:text-muted/50 focus:outline-none disabled:opacity-60"
          />
          <button
            type="submit"
            aria-label="Subscribe"
            disabled={loading}
            className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-white/50 text-white transition-colors hover:bg-white/15 disabled:opacity-60"
          >
            {loading ? (
        ...
        <div className="mt-4 h-5 font-mono text-xs">
          {status === 'error' && <p className="text-white">// {message}</p>}
          {done && <p className="text-neon">// {message}</p>}
        </div>
      </motion.div>
    </section>
  )
