# Innovators Quest — Neo-City Home Page
...
The site was running two WebGL canvases (hero + about) continuously at high DPI,
which caused stutter. Fixes applied:

- **Off-screen canvases stop rendering.** Each `ChipCanvas` uses an
  IntersectionObserver to set the R3F `frameloop` to `never` when it isn't on
  screen, so only the visible chip is drawing. (`src/three/ChipCanvas.jsx`)
- **Pixel ratio capped** to `dpr={[1, 1.5]}` so high-DPI screens don't push ~4x
  the pixels through bloom.
- **One shared cursor listener** for all floating cards instead of one each
  (`src/lib/pointer.js` + `src/components/Floaty.jsx`).
- **Code-split bundles** — three.js is its own cached chunk and the non-home
  pages lazy-load (`vite.config.js`, `src/App.jsx`).

If it's still heavy on a low-end device, lower `dpr` to `1` and/or reduce the
`<Bloom intensity>` in `ChipCanvas.jsx`, or drop the hero chip by removing the
`<ChipCanvas>` in `src/components/Hero.jsx`.
